...
class AdjacencyList {
  private:
    ...
    std::unordered_map<std::string, int> idToIndex;// 學號轉整數索引
    std::vector<std::string> indexToID;// 整數索引轉學號
    std::vector<std::vector<int>> adjListIdx;// 整數版的鄰接表，方便快速 BFS
    std::vector<std::vector<bool>> memo_reachable;// 二維布林陣列備忘錄：[i][j] 表示 i 是否可達 j
    std::vector<bool> hasComputed;// 記錄該整數索引是否已經完整跑過 BFS

    void SortPutID();
    void SortAdjNode(std::vector<AdjacencyNode> &temp);
    void Sort();

    void Connection(int startIdx);
    
    void DFS(std::string cur, float limit, std::unordered_set<std::string> &visited);
  ...
  public:
    ...
    void ComputeInfluence(float limit, std::string num);//任務三
    void ComputeInfluence(float limit); //任務四
    ...
    while(true) {
        ...
        if (verb == "0") break;
            ...
            if (adjacencyList.getListSize() == 0) {
                ...
                std::cout << "\n[Elapsed time] " << duration.count() << " ms" << std::endl << std::endl;
            }
        } else if (verb == "3") {
            if (adjacencyList.getListSize() == 0) {
                std::cout << "### There is no graph and choose 1 first. ###\n";
                ...
                while (std::cin >> temp) {
                    if (IsDouble(temp)) {
                        limit = std::stof(temp);
                        if (limit < 0.66 || limit > 1.0) {
                            std::cout << "\n### It is NOT in [0.66,1.0] ###\n";
                        } else break;
                    }
                    std::cout << "\nInput a real number in [0.66,1.0]: ";
                }
                adjacencyList.ComputeInfluence(limit, num);
            }
        } else if (verb == "4") {
            if (adjacencyList.getListSize() == 0) {
                std::cout << "### There is no graph and choose 1 first. ###\n";
                ...
                auto start = std::chrono::high_resolution_clock::now();
                adjacencyList.ComputeInfluence(0.66);
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
                std::cout << "\n[Elapsed time] " << duration.count() << " ms" << std::endl << std::endl;
...
void AdjacencyList::SortPutID() {
    ...
    // ⚡ 建立雙向學號與索引對照表
    indexToID.resize(listSize);
    for (size_t i = 0; i < sender.size(); i++) {
        idToIndex[sender[i].ID] = i;
        indexToID[i] = sender[i].ID;
    }

    // ⚡ 預建整數鄰接表，完全避開字串查找
    adjListIdx.assign(listSize, std::vector<int>());
    for (size_t i = 0; i < sender.size(); i++) {
        std::string srcID = sender[i].ID;
        for (const auto& edge : adjList[srcID]) {
            adjListIdx[i].push_back(idToIndex[edge.ID]);
        }
    }
...
void AdjacencyList::Connection(int startIdx) {
    if (hasComputed[startIdx]) return; 

    std::queue<int> q;
    std::vector<bool> &visited = memo_reachable[startIdx]; 
    ...
    while (!q.empty()) {
        ...
        // ⚡【終極剪枝】：若該鄰居節點之前已跑過完整 BFS，直接把它的可達矩陣聯集過來並中斷該分支
        if (cur != startIdx && hasComputed[cur]) {
            for (int i = 0; i < listSize; i++) {
                if (memo_reachable[cur][i]) {
                    visited[i] = true;
                }
            }
            continue; 
        ...
        for (int nextIdx : adjListIdx[cur]) {
            if (!visited[nextIdx]) {
    ...
    sender[startIdx].connectCount = count;
}

void AdjacencyList::DFS(std::string cur, float limit, std::unordered_set<std::string> &visited) {
    std::vector<AdjacencyNode> adjNodes = adjList[cur];
    for (int i = 0; i < adjNodes.size(); i++) {
        if (adjNodes[i].weight >= limit) {//有效邊
            std::string nextID = adjNodes[i].ID;
            if (visited.find(nextID) == visited.end()) {
                visited.insert(nextID);
                DFS(nextID, limit, visited); //遞迴深入走訪
            }
        }
    }
}
...
void AdjacencyList::WriteFileInf(std::string num) {
    ...
    for (int i = 0; i < infsender.size(); i++) {
        fout << "[" << std::setw(3) << i + 1 << "] " << infsender[i].ID 
    ...
    fout.close();
    std::cout << "\n<<< There are "<< infsender.size() <<" IDs in total. >>>" << std::endl;
}
...
void AdjacencyList::CreatList(std::vector<Edge> &Info) {
    for (size_t i = 0 ; i < Info.size() ; i++) { 
        ...
        if (adjList.find(cur.putID) == adjList.end()) { 
            PutID toAdd;
            strcpy(toAdd.ID, cur.putID);
            sender.push_back(toAdd);
        }
        adjList[cur.putID].push_back(temp); 

        if (adjList.find(cur.getID) == adjList.end()) { 
            adjList[cur.getID];
            PutID toAdd2;
    ...
    // ⚡ 初始化優化結構
    memo_reachable.assign(listSize, std::vector<bool>(listSize, false));
    hasComputed.assign(listSize, false);
...
void AdjacencyList::WriteFileCnt(std::string num) {
    ...
    for (size_t i = 0 ; i < sender.size() ; i++) {
        ...
        // ⚡ 直接從矩陣轉回學號清單並單獨排序，省下 RBT 紅黑樹動態配置耗時
        int srcIdx = idToIndex[sender[i].ID];
        std::vector<std::string> localConnectList;
        for (int j = 0; j < listSize; j++) {
            if (memo_reachable[srcIdx][j] && j != srcIdx) {
                localConnectList.push_back(indexToID[j]);
            }
        }
        std::sort(localConnectList.begin(), localConnectList.end());

        int j = 0;
        for (const std::string &id : localConnectList) {
            fout << "\t(" << std::setw(2) << j + 1 << ") " << id;
            if ((j + 1) % 12 == 0 && j != 0) fout << std::endl;
            j++;
        }
...
void AdjacencyList::ComputeInfluence(float limit, std::string num) {
    infsender.clear();
    for (int i = 0; i < sender.size(); i++) {
...
void AdjacencyList::ComputeInfluence(float limit) {
    infsender.clear();
    for (int i = 0; i < sender.size(); i++) {
        sender[i].infList.clear();
        std::unordered_set<std::string> visited;
